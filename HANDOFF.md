...
ブロックは固定 head の後、直近会話の前に置きます。Lite固有の自動要約は正式リリース前に
退役し、Chronicle / Memopedia へ移行する前提にしました。起動・バックアップ復元・native
import の各入口で旧 `summary` 行を削除し、手書きと外部サービス由来の note だけを扱います。
