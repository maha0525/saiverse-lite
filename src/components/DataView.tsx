  ...
  return (
    <section className="view content-view" aria-labelledby="data-title">
      ...
      <div className="data-grid">
        ...
        <article className="panel data-card"><span className="card-icon" aria-hidden="true"><Import size={20} strokeWidth={1.8} /></span><h2>これまでの会話を連れてくる</h2><p>ChatGPT / Claude の公式エクスポート（JSON / ZIP、zip のままで OK）に対応。Claude は本人が持っていた記憶（memories.json）も一緒に連れてきます。</p><div className="button-stack"><FileAction label="ChatGPT から" accept="application/json,.json,application/zip,.zip" disabled={props.busy} onFile={props.onImportChatGpt} /><FileAction label="Claude から" accept="application/json,.json,application/zip,.zip" disabled={props.busy} onFile={props.onImportClaude} /></div></article>
      </div>
    </section>
  );
}
