...
export function MemoryView({ persona, memories, onCreate, onEdit, onDelete }: MemoryViewProps) {
  ...
  return (
    <section className="view content-view" aria-labelledby="memory-title">
      <header className="view-header"><div><span className="eyebrow">MEMORY</span><h1 id="memory-title">{persona.name}の記憶</h1><p>保存した記憶を、ここで見て直せます。</p></div><span className="count-badge">{memories.length}件</span></header>
      ...
      <div className="memory-grid">
        {memories.map((memory) => (
          <article className="memory-card" key={memory.id}>
            <div className="memory-card-head"><span className="memory-kind">記憶</span><time>{new Intl.DateTimeFormat("ja-JP", { dateStyle: "medium", timeStyle: "short" }).format(memory.updatedAt)}</time></div>
            <p>{memory.content}</p>
        ...
        {memories.length === 0 && <div className="empty-state compact-empty"><h2>まだ記憶はありません</h2><p>覚えていてほしいことを手書きで追加できます。</p></div>}
      </div>
    </section>
  );
}
