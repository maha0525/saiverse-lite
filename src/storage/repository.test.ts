...
async function exercise(repository: LiteRepository): Promise<void> {
  ...
  const summary: MemoryEntry = { ...memory, id: newId("memory"), kind: "summary", content: "summary", sourceMessageIds: [message.id] };
  ...
  await repository.putMemory(summary);
  ...
  expect(await repository.listMemories(persona.id)).not.toContainEqual(summary);
  expect(await repository.listMemories(persona.id)).toContainEqual(memory);
  await repository.putMessage(message);
